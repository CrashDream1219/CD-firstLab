...
import ohos.codelabs.distributedvideo.duiagent.DuiAgent;
...
public class RemoteController extends DependentLayout
    ...
    private int defaultid = ResourceTable.Layout_remote_controller;
     ...
     */
    public RemoteController(Context context, DuiAgent agt) {
        super(context);
        this.context = context;
        this.agt = agt;
        this.defaultid = agt.getter_id(); //ResourceTable.Layout_remote_controller
        initView();
    }
    ...
    public void initView() {
        setVisibility(INVISIBLE);
            componentParent = LayoutScatter.getInstance(context).parse(
                    ...
                    this,
                    false);
        Button button1 = initButton(ResourceTable.Id_remote_resume);
        Button button2 = initButton(ResourceTable.Id_remote_pause);
        Button button3 = initButton(ResourceTable.Id_remote_stop);
        Button button4 = initButton(ResourceTable.Id_remote_forward);
        Button button5 = initButton(ResourceTable.Id_remote_reward);
        Button button6 = initButton(ResourceTable.Id_remote_volume_up);
        Button button7 = initButton(ResourceTable.Id_remote_volume_down);
        if (agt.NeedChange){
            ShapeElement buttonPara = agt.seletor().get_shape();
            button1.setBackground(buttonPara);
            button2.setBackground(buttonPara);
            button3.setBackground(buttonPara);
            button4.setBackground(buttonPara);
            button5.setBackground(buttonPara);
            button6.setBackground(buttonPara);
            button7.setBackground(buttonPara);
            agt.toggle_player_selector();
        }
    ...
    private Button initButton(int res) {
        Button button1 = null;
        if (componentParent.findComponentById(res) instanceof Button) {
            button1 = (Button) componentParent.findComponentById(res);
            button1.setClickedListener(this);
            button1.setTouchEventListener(this);
