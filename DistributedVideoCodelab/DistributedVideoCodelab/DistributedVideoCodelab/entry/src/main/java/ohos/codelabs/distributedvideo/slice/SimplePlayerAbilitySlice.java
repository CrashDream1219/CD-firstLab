...
import ohos.codelabs.distributedvideo.duiagent.*;
...
public class SimplePlayerAbilitySlice extends AbilitySlice {
    ...
    private ListContainer uiSetContainer;
    private Image tv;
    private SlidePopupWindow transWindow;
    private SlidePopupWindow transWindowTwo;
    ...
    public void onStart(Intent intent) {
        ...
        this.agt = new DuiAgent(PresetTable.Player_Default);
        agt.upload_set();
        initComponent();
        initListener();
    }
    ...
    private void initComponent() {
        if (findComponentById(ResourceTable.Id_parent) instanceof DependentLayout) {
            ...
            transWindowTwo = new SlidePopupWindow.Builder(this).create(ResourceTable.Layout_ui_slide);
            ...
            if (transWindowTwo.findComponentById(ResourceTable.Id_ui_list_container) instanceof ListContainer) {
                uiSetContainer =
                        (ListContainer) transWindowTwo.findComponentById(ResourceTable.Id_ui_list_container);
            }
            remoteController = new RemoteController(this,agt);
            ...
            parentLayout.addComponent(transWindowTwo);
    ...
    private void showDeviceList() {
        CommonProvider<DeviceInfo> commonProvider = new CommonProvider<DeviceInfo>(
            ...
            protected void convert(ViewProvider holder, DeviceInfo item, int position) {
                holder.setText(ResourceTable.Id_device_text, item.getDeviceName()); //213213
    ...
    private void SelectUIList(){// Show distributable UI list for choosing
        List<String> x = agt.getter1();
        FirstProvider<String> commonProvider2 = new FirstProvider<String>(x, getContext(), ResourceTable.Layout_dui_list) {
            @Override
            protected void converts(DuiViewProvider holder, String item, int position) {
                holder.setText(ResourceTable.Id_ui_text, item);
                Button clickButton = holder.getView(ResourceTable.Id_ui_text);
                clickButton.setText(item);
                clickButton.setClickedListener(component -> {
                    transWindowTwo.hide();
                    if(!Objects.equals(item, agt.Select())){
                        agt.register(item);
                        agt.player_select();
                        re_init();
                    }
        ...
        uiSetContainer.setItemProvider(commonProvider2);
        commonProvider2.notifyDataChanged();
        transWindowTwo.show();
